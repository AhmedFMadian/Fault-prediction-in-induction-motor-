...
# =========================
# =========================
# NEW DNN MODEL ARCHITECTURE
# =========================
def dnn_branch(input_shape, prefix):
    """
    Replaces cnn_branch. Focuses on deep feature extraction
    from the targeted PSD bins using dense connections.
    """
    l2 = regularizers.l2(1e-4)
    inp = layers.Input(shape=input_shape, name=f"{prefix}_input")

    # Flatten the (bins, 1) input to (bins,)
...
def build_model(input_dim, n_classes):
    """
    Updates the fusion logic to handle the new DNN branches.
    """
    shape = (input_dim, 1)
    v_in, v_feat = dnn_branch(shape, "vib")
    c_in, c_feat = dnn_branch(shape, "cur")

    # Fusion of Vibration and Current features
    fused = layers.Concatenate(name="fusion")([v_feat, c_feat])

    # Deep Head
    x = layers.Dense(128, activation="relu", kernel_regularizer=regularizers.l2(1e-4))(fused)
    x = layers.BatchNormalization()(x)
    ...
    model = Model([v_in, c_in], out, name="RAP_MSF_DNN_v13")
    model.compile(
...
def mc_predict(model, X_vib, X_cur, T=MC_PASSES):
    # Ensure the model is called in training mode to keep dropout active
    preds = np.stack(
    ...
    entropy = -np.sum(mu * np.log(mu + 1e-10), axis=1)
    return mu, entropy
# =========================
# VISUALISATION
# =========================
